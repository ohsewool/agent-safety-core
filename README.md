...
```bash
python3 -m pytest tests/ -q          # 550 tests
...
지금은 **분기 커버리지** 100%이고, CI가 그것을 관문으로 지킨다. 처음엔 구문 커버리지로 걸었는데, **줄이 전부 실행됐다는 것과 각 `if`가 양쪽으로 다 가봤다는 것은 다르다** — 다시 재니 네 분기가 한 번도 그 방향으로 가본 적이 없었고, 그중 하나가 "저널 파일은 있는데 비어 있는" 상태였다(만들어놓고 아직 아무것도 안 쓴 순간이다). **관문이지 목표가 아니다** — 새 방어 분기는 테스트를 받거나 `# pragma: no cover`와 이유를 옆에 적는다. 마지막 한 줄이 그렇게 처리됐고, **이유를 적다가 그것이 구조적으로 도달 불가**라는 것이 드러났다(`BEGIN IMMEDIATE`가 그 행을 잡고 있다).
